...
function line_to_methrec( fields, data_cols, round )
    if length(fields) == 1
        return MethRecord( EOFMarker(), nocall )
    end
    gp = GenomicPosition( fields[data_cols.chrom], parse( UInt32, fields[data_cols.pos] ) )
    meth_frac = parse( Float64, fields[data_cols.meth_frac] )
    n_obs = parse( Int, fields[data_cols.n_obs] )
    if meth_frac == 0
        if n_obs == 0
            call = nocall
        else
...
function main(methcalls_dir::String, output::String, data_cols::DataCols; 
    ...
    # 1 - chromosome
    # 2 - position
    # 3 - fraction of methylated
    # 4 - number of observations reads
